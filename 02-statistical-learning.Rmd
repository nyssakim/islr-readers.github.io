...
There are two main reasons to estimate $f$: - Prediction - Inference
...
\$ Y \approx \beta\_0 + \beta\_1 X_1 + \beta\_2 X_2 + ... + \beta\_p X_p\$
...
Potential Cons:\
- We don't know the true $f$, and we could be way off! - We can choose more flexible models to address this, but... - More flexible models lead to more parameters to estimate, and potentially **overfitting**.

### **Non-parametric Methods**

Pro: Do not make assumptions about functional form. Con: Require a large number of observations to obtain an estimate of $f$
...
Note: - Clustering methods are imprecise -- They cannot assign all points to their correct group. - If there are $p$ variables, then $)p(p- 1) /2)$ scatterplots can be made, this is why automated clustering methods are important.\
